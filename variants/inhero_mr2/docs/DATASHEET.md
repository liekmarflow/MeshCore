...
| **JEITA Override** | `set board.jeitaignore 1` sets the BQ25798 TS_IGNORE bit, so charging continues below the T-Cold threshold (≈ −2 °C). Off by default. Accepted for Li-ion 1S and LiFePO4 1S; requires explicitly set `board.batcap` and `board.imax` strictly below 0.05C of that capacity; gate violations are rejected without changing existing values. With TS_IGNORE set the charge suspend at T-Hot (≈ +57.7 °C) is dropped as well. Frost charging Li-ion or LiFePO4 is at the operator's own risk and is scoped to Central European frost; sites regularly below −20 °C call for LTO or a Na-ion cell rated for charging there. See [BATTERY_GUIDE.md](BATTERY_GUIDE.md). |
