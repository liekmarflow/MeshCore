...
For Li-ion and LiFePO4, charging in the **T-Cool range** (+3 °C to −2 °C with the Inhero voltage divider) is blocked by default and can be set to a reduced rate via `set board.fmax` (20%, 40% or 100%). `fmax` acts in this band only — below −2 °C (T-Cold) the hardware block stands regardless of `fmax`, and only `board.jeitaignore` lifts it. Changing chemistry resets the battery and charging settings; selecting the same chemistry preserves them. [FAQ #6](FAQ.md#6-what-is-frost-charging-and-how-do-fmax-and-jeitaignore-work-together) shows the two settings side by side.
