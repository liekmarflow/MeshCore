...
# Storage and the gate check use one decimal place (mAh).
...
#  "Err: Failed to store setting"                 — storage or hardware setup failed
...
`get board.saved` is removed. Use the individual getters; there is no additional format version.
