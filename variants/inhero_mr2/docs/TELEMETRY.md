...
Die Inhero MR-2 sendet Telemetriedaten im [CayenneLPP](https://docs.mydevices.com/docs/lorawan/cayenne-lpp)-Format über drei Kanäle. Die Companion-App zeigt diese als **Kanal 1–3** an.

---

## Kanal 1 — Gerätestatus & Umgebung

Basisdaten des Nodes, ergänzt durch den BME280-Umgebungssensor.

| Feld | Einheit | Quelle | Beschreibung |
|------|---------|--------|--------------|
| Akkustand | V | nRF52 ADC | Batteriespannung (Rohwert vom internen ADC). Die angezeigte Prozentangabe wird von der App aus der Spannung geschätzt. |
...
> **Hinweis:** Ohne BME280 wird stattdessen die MCU-interne Temperatur des nRF52840 angezeigt. Die Höhenberechnung basiert auf dem Standard-Meeresspiegeldruck (1013,25 hPa) und kann je nach Wetterlage abweichen.
...
| Feld | Einheit | Quelle | Beschreibung |
|------|---------|--------|--------------|
| Volt | V | INA228 | Batteriespannung (24-Bit-ADC, ±0,1 % Genauigkeit) |
| SOC | % | INA228 | Ladezustand per Coulomb-Counting — *optional, nur wenn kalibriert* |
| Aktuell | A | INA228 | Batteriestrom. Negativ = Entladung, positiv = Ladung |
| Temperatur | °C / °F | BQ25798 NTC | Batterietemperatur am NTC-Fühler |
| TTL | Tage | berechnet | Geschätzte Restlaufzeit — *optional, nur bei gültigem SOC* |

> **Hinweis:** SOC und TTL erscheinen nur, wenn der Coulomb-Counter ausreichend kalibriert ist (nach mindestens einem vollständigen Lade-/Entladezyklus). Solange der SOC noch nicht gültig ist, werden diese Felder weggelassen.
...
| Feld | Einheit | Quelle | Beschreibung |
|------|---------|--------|--------------|
| Volt | V | BQ25798 | Spannung am Solareingang (VBUS) |
| Aktuell | A | BQ25798 | Solarstrom |
| Prozent | % | Firmware | MPPT-Aktivierung der letzten 7 Tage. Zeigt, wie viel Prozent der Zeit der MPPT-Regler aktiv Solar-Energie eingespeist hat. |
...
┌─────────────────────────────────────────────┐
│ Kanal 1: Spannung (nRF52 ADC)              │  ← MyMesh.cpp
...
│ Kanal 1: MCU-Temperatur (Fallback)         │  ← MyMesh.cpp
└─────────────────────────────────────────────┘
```
