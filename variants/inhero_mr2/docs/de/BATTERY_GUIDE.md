...
Für Li-ion und LiFePO4 ist die Ladung im **T-Cool-Bereich** (+3 °C bis −2 °C mit Inhero-Spannungsteiler) standardmäßig gesperrt und kann über `set board.fmax` (20%, 40% oder 100%) auf eine reduzierte Rate gesetzt werden. `fmax` wirkt nur in diesem Band — unterhalb von −2 °C (T-Cold) bleibt die Hardware-Sperre unabhängig von `fmax` bestehen, und nur `board.jeitaignore` hebt sie auf. Ein tatsächlicher Chemiewechsel setzt Akku- und Ladewerte zurück; dieselbe Chemie erneut zu setzen erhält sie. [FAQ #6](FAQ.md#6-was-ist-frostladen-und-wie-wirken-fmax-und-jeitaignore-zusammen) stellt beide Einstellungen nebeneinander.
