...
Die Kapazität wird wie bisher mit einer Nachkommastelle (mAh) gespeichert. Das Gate prüft diesen gespeicherten Wert; `get board.batcap` zeigt auf ganze mAh gerundet an.
...
Gate-Abweisungen mit `N/A, …` erfolgen vor Änderungen an Einstellungen oder Hardware.
...
Der MR2 hat keine Backup-Batterie: VDD und VBACKUP liegen an derselben 3,3-V-Versorgung. Bei der RTC-Initialisierung wird in Control 1 (`0x0F`) `EERD=1` gesetzt; nach der EEPROM-Busy-Prüfung werden in Backup (`0x37`) `BSM=00`, `TCE=0`, `BSIE=0` und `FEDE=1` gesetzt. EEOffset[0] und TCR bleiben erhalten (`(alt & 0x83) | 0x10`). Damit sind Backup-Umschaltung und Erhaltungsladung ausgeschaltet. EERD verhindert, dass ein EEPROM-Refresh die RAM-Konfiguration überschreibt, und bleibt bei der Wake-Timer-Programmierung gesetzt (`0x08` / `0x0F` in Control 1). Das EEPROM selbst wird nicht programmiert. Siehe [RV-3028-C7 Application Manual](https://www.microcrystal.com/fileadmin/Media/Products/RTC/App.Manual/RV-3028-C7_App-Manual.pdf).
